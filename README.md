...
## 📋 Графічний інтерфейс (GUI - Лабораторна робота №2)

У папці `gui/` реалізовано повноцінний графічний інтерфейс для керування завданнями у стилі **Kanban-дошки** з використанням бібліотеки **PySide6**.

### 🌟 Ключові можливості
1. **Інтерактивна Kanban-дошка**:
   - Три колонки статусів: **To Do**, **In Progress** та **Done**.
   - Кожна колонка містить лічильник завдань у верхній частині та кнопку швидкого додавання `+`.
2. **Drag & Drop (Перетягування картками)**:
   - Завдання стилізовані під кольорові квадратні "листочки" із заокругленими кутами та drop-shadow тінню.
   - Ви можете затиснути будь-яку картку мишею і перетягнути в іншу колонку — статус завдання в базі даних `tasks.json` оновиться автоматично.
3. **Пріоритезація з кольоровим кодуванням**:
   - Картки візуально маркуються кольоровою межею відповідно до пріоритету:
     - 🔴 **High** (червона рамка, рожевий фон)
     - 🟡 **Medium** (помаранчева рамка, пісочний фон)
     - 🔵 **Low** (блакитна рамка, синій фон)
4. **Контекстне керування та валідація**:
   - Подвійний клік на картку відкриває форму редагування.
   - Правий клік миші відкриває спливаюче контекстне меню (Edit/Delete).
   - Вбудована перевірка введення: спроба зберегти пусту назву завдання або текст довжиною > 100 символів покаже інтерактивне повідомлення про помилку.
5. **Динамічний пошук (Live Filter)**:
   - Рядок пошуку у верхній панелі миттєво відфільтровує картки за ключовими словами в назвах чи описах у реальному часі.

### 🚀 Запуск GUI

Для запуску графічного інтерфейсу виконайте команду з кореневої директорії:

```bash
python3 -m intpy.gui.main
```

Або, якщо ви використовуєте uv:

```bash
uv run python -m intpy.gui.main
```
